...
import {
  ...
  atualizarFormulario,
  deletarFormulario,
  // Importe aqui suas funções para buscar usuarios, obras e titulares
  // Ex: listarUsuarios, listarObras, listarTitulares
...
export const Dashboard = () => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const [dadosFormularios] = await Promise.all([
        listarFormularios(),
        // TODO: Descomente e conecte suas APIs reais aqui quando criar os endpoints
        // listarUsuarios().then(setListaUsuarios),
        // listarObras().then(setListaObras),
        // listarTitulares().then(setListaTitulares),
      ]);

      setRequests(dadosFormularios);

      // ! MOCK TEMPORÁRIO: Se ainda não tiver API de usuarios/obras,
      // remova isso quando tiver o backend pronto para essas tabelas.
      // setListaObras([{ id: 43, nome: "Obra Alpha (API)" }]);
    } catch (error) {
      console.error(error);
  ...
  // Isso garante que a tabela saiba traduzir ID -> Nome baseada nas listas carregadas
  ...
  // --- Handlers de Tabela (Mantidos iguais) ---
      ...
      <div className="max-w-[1800px] mx-auto">
        ...
        <div className="bg-white rounded-2xl shadow-md p-5 mb-6 border border-gray-100">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-4 items-end">
            ...
            {/* Titular (Agora usa lista dinâmica) */}
            <div className="flex flex-col gap-1">
              <label className="text-xs font-semibold text-gray-500 uppercase">
                Titular
              </label>
        ...
        {isLoadingData ? (
          ...
          <PaymentTable
            // !!! IMPORTANTE: Seu PaymentTable deve ser atualizado para aceitar 'columns' e 'expandedFieldsConfig' como props
            columns={columns}
            expandedFieldsConfig={expandedFieldsConfig}
            // Props normais abaixo
